...
class BranchesControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create branch" do
    assert_difference("Branch.count") do
      post branches_url, params: { branch: { cooperative_id: @branch.cooperative_id, name: @branch.name } }
    end
  ...
  test "should update branch" do
    patch branch_url(@branch), params: { branch: { cooperative_id: @branch.cooperative_id, name: @branch.name } }
